...
def predict_labels_multi_scale(images,
  ...
  for output in sorted(outputs_to_predictions):
    ...
    # ----- use logits -------
    prediction = tf.nn.softmax(predictions, 3)
    shape = tf.shape(prediction)
    prediction = tf.slice(prediction, [0, 0, 0, 1], [shape[0],shape[1], shape[2], 1])
    # ----- use argmax --------
    #prediction = tf.argmax(predictions, 3)

    outputs_to_predictions[output] = tf.scalar_mul(255, prediction)
...
def predict_labels(images, model_options, image_pyramid=None):
  ...
  for output in sorted(outputs_to_scales_to_logits):
      ...
      # ----- use logits -------
      prediction = tf.nn.softmax(logits, 3)
      shape = tf.shape(prediction)
      prediction = tf.slice(prediction, [0, 0, 0, 1], [shape[0],shape[1],shape[2], 1])
      # ----- use argmax --------
      # prediction = tf.argmax(logits, 3)

      predictions[output] = tf.scalar_mul(255, prediction)
