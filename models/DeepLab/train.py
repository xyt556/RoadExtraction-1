...
flags.DEFINE_integer('save_interval_secs', 3000,
...
flags.DEFINE_integer('learning_rate_decay_step', 12000,
...
flags.DEFINE_multi_integer('train_crop_size', [512, 512],
                           'Image crop size [height, width] during training.')

flags.DEFINE_float('last_layer_gradient_multiplier', 7.0,
...
flags.DEFINE_float('max_scale_factor', 2.0,
...
def main(unused_argv):
  ...
  with tf.Graph().as_default() as graph:
    ...
    if FLAGS.save_summaries_images:
        ...
        summary_label = tf.cast(tf.scalar_mul(255, graph.get_tensor_by_name(
            ('%s/%s:0' % (first_clone_scope, common.LABEL)).strip('/'))),
            tf.uint8)
        summaries.add(tf.summary.image('samples/%s' % common.LABEL, summary_label))

        predictions = tf.cast(tf.scalar_mul(255, tf.expand_dims(tf.argmax(graph.get_tensor_by_name(
    ...
    # org
    with tf.device(DEVICE):
      if FLAGS.learning_policy=='step' or FLAGS.learning_policy=='poly':
        ...
        optimizer = tf.train.AdamOptimizer(FLAGS.base_learning_rate, 0.9, 0.999)
    '''
